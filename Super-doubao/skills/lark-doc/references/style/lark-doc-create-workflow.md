...
1. 分析用户需求：受众、目的、**体裁**（信息型 / 议论 / 随笔 / 应用文…）、范围
2. 设计大纲——**按体裁判断每节是否需要非文本 block**：承载数据 / 对比 / 流程等结构化信息的章节优先规划画板；纯论证、叙事章节保持文字即可，不强制每节配块
3. `docs +create --api-version v2` **只建骨架**：标题 + 各级标题 + 每节一句占位摘要（信息型文档可在开头加 `<callout>`，非强制）
...
6. 评估样式是否**适配内容**（结构化信息是否用了合适的 block；有无为凑数硬加的图表）——不套用固定密度门槛
7. **画板意图识别**：逐章节扫描，按 `lark-doc-style.md`「画板意图识别」表，用「重要度 + 适配度」双重判断是否有段落真正适合用图表达，记录需要插图的章节、推荐画板类型、mermaid/SVG 路径和用于画图的源内容
...
9. Spawn 内容改写 Agent 定向润色：
   - **仅当章节确含结构化信息时**，把密集文字转为 `<table>`/`<grid>`/`<callout>`；纯论证 / 叙事段落保持文字
   - 长文档主要章节间可补充 `<hr/>`（非强制）
