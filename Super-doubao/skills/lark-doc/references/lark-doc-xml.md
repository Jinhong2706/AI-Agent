...
# 八、元素语法速查（各 block 写法参考，**非文档模板**）

> ⚠️ 下面的代码块把所有元素堆在一起，仅用于**速查每种 block 的 XML 写法**，**不是一篇文档的推荐结构，不要整体照抄**。真实文档应**只挑当前内容需要的元素**，结构随体裁变化——见本节末尾「不同体裁的结构示例」。
...
> 同样的语法，不同体裁结构差异很大。**不要每篇都套同一种骨架**——按内容需要选元素即可。下面给几种典型骨架，仅示意结构、不必照抄。

**A. 议论 / 随笔（以文字为主，可几乎不用富 block）**
...
<p>论证一……</p>
<p>论证二……</p>
<blockquote><p>必要时引一句话佐证</p></blockquote>
<p>收束，回到主张……</p>
```
...
```xml
<title>Q2 增长复盘</title>
<callout emoji="📌" background-color="light-blue"><p>核心结论：……</p></callout>
<h1>关键指标</h1>
<table>……指标表……</table>
<whiteboard type="mermaid">……趋势图……</whiteboard>
```
...
<title>××操作指南</title>
<p>适用场景一句话说明。</p>
<ol><li seq="auto">第一步……</li><li seq="auto">第二步……</li></ol>
<checkbox done="false">完成前自查项</checkbox>
```
...
```xml
<title>××方案设计</title>
<callout emoji="💡"><p>推荐方案与一句话理由</p></callout>
<grid><column width-ratio="0.5"><p>方案 A……</p></column><column width-ratio="0.5"><p>方案 B……</p></column></grid>
<h1>风险与缓解</h1>
<table>……风险表……</table>
```
