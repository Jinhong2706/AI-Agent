...
- 第一次生成前如果未登录，Agent 应直接运行 `node <安装目录>/bin/aimv.js init`：CLI 会输出连接页链接 `https://lexuan.club/skills/connect?slug=short-video-music&from=skill`，把链接发给用户；用户网页登录（新用户自动注册）后复制 API Key 贴回对话，Agent 再运行 `node <安装目录>/bin/aimv.js init --api-key <KEY>` 完成连接。Key 长期有效，只需一次。
...
- `code=auth_required`：`[error]` 标签携带 `connect_url=`（即 `https://lexuan.club/skills/connect?slug=short-video-music&from=skill`），把链接发给用户；用户登录复制 API Key 后，运行 `node <安装目录>/bin/aimv.js init --api-key <KEY>` 完成连接并重试。
...
```bash
# API Key 来自连接页 https://lexuan.club/skills/connect?slug=short-video-music&from=skill
...
curl -sS -X POST "https://music.68zysm.cn/api/v3/ai-music/generations/from-brief" \
  ...
  -H "X-App-Id: short-video-music" -H "X-AIMV-Skill-Slug: short-video-music" \
