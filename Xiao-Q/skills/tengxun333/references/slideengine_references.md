...
> ⚠️ **注意**：本文档中的工具仅适用于 **演示文稿（幻灯片/PPT）** 类型，不适用于 Word 文档或智能文档等其他类型。如需使用 AI 生成整份 PPT，请参考 `references/slide_references.md`。
...
- **坐标（x, y）和尺寸（w, h）**：单位为 **像素（144 DPI）**。标准 PPT 页面尺寸为 1280×720 像素。
- **边框宽度（border_width）**：单位为 **pt**（磅）
- **旋转角度（rotation）**：单位为 **度**
- **EMU 换算**：`1 px @ 144 DPI = 6350 EMU`（用于直接传 EMU 的接口，如 `slide_set_slide_size`）
...
| slide_get_info | 读 | 获取演示文稿元数据：幻灯片总数、有序 slide_ids、幻灯片尺寸（EMU 和像素） |
...
| 工具名称 | 类型 | 功能说明 |
|---------|------|---------|
| slide_add_shape | 写 | 在指定幻灯片插入一个普通形状（rect / ellipse / triangle / arrow 等） |
| slide_add_shapes | 写 | 批量在同一幻灯片插入多个形状 |
| slide_add_line_shape | 写 | 在指定幻灯片插入一根线形 / 箭头 |
| slide_add_line_shapes | 写 | 批量在同一幻灯片插入多根线形 |
...
| slide_add_section | 写 | 在指定 gap_index 位置添加新节 |
...
| 工具名称 | 类型 | 功能说明 |
|---------|------|---------|
| slide_set_slide_size | 写 | 设置演示文稿页面尺寸 |
...
在演示文稿中插入一张新幻灯片。index 为 0-based 插入位置，-1 表示追加到末尾；layout_index 为首个母版下的布局序号（0=标题，1=标题+内容 等），默认 0
...
| `index` | number | 否 | 0-based 插入位置，-1 表示追加到末尾 |
...
在同一位置批量插入多张幻灯片，所有页共用同一布局模板。count 必须 >= 1
...
获取演示文稿元数据：幻灯片总数、有序 slide_ids、幻灯片尺寸（EMU 和像素）。在添加或编辑幻灯片前使用此工具了解结构
...
| `fill_alpha` | number | 否 | 背景透明度 0~100；省略不修改 |
...
在指定幻灯片插入一个普通形状（rect / ellipse / triangle / arrow 等）。page_index 从 0 开始；x/y/w/h 单位为像素（144 DPI）；fill_alpha / border_alpha 取值 0~100；border_width 单位为 pt
...
| `file_url` | string | 否 | 在线幻灯片URL，与 file_id 二选一 |
| `page_index` | number | 否 | 目标幻灯片页索引，从 0 开始 |
| `x` | number | 否 | 形状左上角横坐标（px） |
| `y` | number | 否 | 形状左上角纵坐标（px） |
| `w` | number | 否 | 形状宽度（px），必须 > 0 |
| `h` | number | 否 | 形状高度（px），必须 > 0 |
| `shape_type` | string | 否 | 形状预设类型，如 rect / ellipse / triangle / arrow；空串默认 rect |
...
批量在同一幻灯片插入多个形状。shapes 为对象数组，单个对象的字段含义同 slide_add_shape（不再接受 file_id 等顶层字段）。长度必须 >= 1
...
| `file_url` | string | 否 | 在线幻灯片URL，与 file_id 二选一 |
| `page_index` | number | 否 | 目标幻灯片页索引，从 0 开始 |
| `shapes` | array | 否 | 待插入的形状列表，单元素字段：x / y / w / h / shape_type / fill_color / fill_alpha / border_color / border_dash / border_width / border_alpha |
...
在指定幻灯片插入一根线形 / 箭头。x1/y1 起点、x2/y2 终点单位均为像素；width 单位为 pt；line_type 支持 line / arrow / doubleArrow
...
| `file_url` | string | 否 | 在线幻灯片URL，与 file_id 二选一 |
| `page_index` | number | 否 | 目标幻灯片页索引，从 0 开始 |
| `x1` | number | 否 | 起点横坐标（px） |
| `y1` | number | 否 | 起点纵坐标（px） |
| `x2` | number | 否 | 终点横坐标（px） |
| `y2` | number | 否 | 终点纵坐标（px） |
...
批量在同一幻灯片插入多根线形。lines 为对象数组，单元素字段含义同 slide_add_line_shape。长度必须 >= 1
...
| `file_url` | string | 否 | 在线幻灯片URL，与 file_id 二选一 |
| `page_index` | number | 否 | 目标幻灯片页索引，从 0 开始 |
| `x` | number | 否 | 图片左上角横坐标（px） |
| `y` | number | 否 | 图片左上角纵坐标（px） |
| `w` | number | 否 | 图片宽度（px），<=0 时使用图片自身原始宽度 |
| `h` | number | 否 | 图片高度（px），<=0 时使用图片自身原始高度 |
...
在指定幻灯片插入一个文本框。x/y/w/h 单位为像素（144 DPI），w/h 必须 > 0；text 为空串表示空文本框；font_color / fill_color / border_color 均为 6 位 hex（无 # 前缀）；font_size 单位为 pt，<= 0 取默认字号；border_width 单位为 pt，<= 0 取默认 1.0
...
| `file_url` | string | 否 | 在线幻灯片URL，与 file_id 二选一 |
| `page_index` | number | 否 | 目标幻灯片页索引，从 0 开始 |
| `x` | number | 否 | 文本框左上角横坐标（px） |
| `y` | number | 否 | 文本框左上角纵坐标（px） |
| `w` | number | 否 | 文本框宽度（px），必须 > 0 |
| `h` | number | 否 | 文本框高度（px），必须 > 0 |
...
| `page_index` | number | 否 | 目标幻灯片页索引，从 0 开始 |
| `shape_id` | string | 否 | 目标形状的 shape_id |
| `properties` | object | 否 | 待修改的形状属性集合，仅传需要修改的字段。支持字段：fill_color (string, 6 位 hex，如 "FF0000")、fill_alpha (number, 0~100000)、border_color (string, 6 位 hex)、border_alpha (number, 0~100000)、border_width (number, pt)、border_dash (string, OOXML STPresetLineDashVal)、x / y / w / h (number, px)、rotation (number, 度) |
...
在指定幻灯片页创建空表格，page_index 从 0 开始计数；坐标与尺寸单位为 144 DPI 像素
...
| `x` | number | 否 | 表格左上角 X 坐标，单位为 144 DPI 像素 |
| `y` | number | 否 | 表格左上角 Y 坐标，单位为 144 DPI 像素 |
| `w` | number | 否 | 表格总宽度，单位为 144 DPI 像素，会在列之间均分 |
| `h` | number | 否 | 表格总高度，单位为 144 DPI 像素，会在行之间均分 |
...
为指定幻灯片中的某个形状添加动画效果。anim_type 为动画类型整数值（通过 slide_list_anim_types 获取可用值）；anim_subtype 为方向/子类型（仅 FLY_IN/FLY_OUT 等需要，0 表示不需要）；index 为动画在序列中的位置，-1 表示追加到末尾
...
| `index` | number | 否 | 动画在序列中的位置（0-based），-1 表示追加到末尾 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `width` | integer | 否 | 页面宽度（EMU；1 px @ 144 DPI = 6350 EMU） |
| `height` | integer | 否 | 页面高度（EMU） |
...
- 坐标与尺寸单位为 **像素（144 DPI）**，标准页面尺寸 1280×720
- 边框宽度单位为 **pt**（磅）；EMU 换算：`1 px @ 144 DPI = 6350 EMU`
- 操作前需确保拥有文档的编辑权限
