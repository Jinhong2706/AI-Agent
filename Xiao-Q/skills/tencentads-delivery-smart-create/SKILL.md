...
- **用户未提及任何定向或用户表达意图是智能定向时**（通投）→ `targeting` 传空对象 `{}`，**跳过以下所有定向步骤**，直接进入 6B
- **用户有定向需求或用户表达意图是手动定向时** → 按下方规则构造 `targeting`
...
可查询的定向枚举字段包括：`gender`、`education`、`user_os`、`excluded_os`、`network_type`、`device_price`、`marital_status`、`app_install_status`、`location_types`、`excluded_dimension`、`excluded_day`、`wechat_ad_behavior_actions`、`wechat_ad_behavior_excluded_actions`。
...
| `configured_status` | enum | 否 | 广告状态，默认上线 |
