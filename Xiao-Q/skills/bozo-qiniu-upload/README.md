...
cp -r skills/bozo-qiniu-upload ~/.openclaw/skills/

# 2. 安装依赖
cd ~/.openclaw/skills/bozo-qiniu-upload && npm install
...
在 TsClaw 的 Skills 页面，找到 `bozo-qiniu-upload`，配置以下环境变量：
...
- 「把这张图片上传到网上」
- 「用 bozo-qiniu-upload 上传 /tmp/xxx.png」
...
# 3. 在 Skill 目录下发布
cd skills/bozo-qiniu-upload
clawhub publish . --slug bozo-qiniu-upload --name "Qiniu Upload" --version 1.0.2 --changelog "Fix SKILL.md metadata format for TsClaw parsing"
```
