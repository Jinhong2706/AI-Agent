...
在 `~/.openclaw/openclaw.json` 的 `skills.entries.bozo-qiniu-upload` 中配置环境变量，或在系统环境变量中设置：
...
2. 执行上传脚本：
   ```
   node ~/.openclaw/skills/bozo-qiniu-upload/scripts/upload.js <文件路径>
   ```
   或使用绝对路径调用
