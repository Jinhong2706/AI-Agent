...
class SelfModel {
  ...
  updateCapabilities() {
    // Dynamically discover tools from the actual file system using proper glob
    const { glob } = require('glob');
    const toolFiles = glob.sync(path.join(__dirname, '..', '*.js'));
    const capabilities = {
      tools: toolFiles
        .map(f => path.basename(f, '.js'))
        .filter(n => n !== 'index' && !n.startsWith('_')),
      skills: [
