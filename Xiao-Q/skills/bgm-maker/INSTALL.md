...
这是一个已经打好的 AI背景音乐生成 Node CLI 发布包，里面已经包含运行所需代码。
...
AI背景音乐生成 的通用核心是 `SKILL.md` + 本地 Node CLI。不同 Agent 需要适配的是“如何发现说明”和“如何执行本地命令”，不是音乐生成逻辑。

通用要求：

- 让 Agent 读到本目录的 `SKILL.md`
- 让 Agent 能执行本目录的 `bin/aimv.js`，命令为 `node`
- 触发语义：背景音乐生成、AI背景音乐、BGM生成、配乐生成、纯音乐、视频BGM、游戏配乐、播客配乐
...
发布包内的 `SKILL.md` 已经是 Codex 原生 Skill 格式。复制到 Codex 本地 skills 目录后，重启 Codex 即可自动识别 AI背景音乐生成。执行命令时使用 `node <安装目录>/bin/aimv.js`。

macOS / Linux:

```bash
CODEX_SKILL_DIR="$HOME/.codex/skills/bgm-maker"
...
```powershell
$CodexSkillDir = Join-Path $env:USERPROFILE ".codex\skills\bgm-maker"
...
{
  "name": "aimv",
  "description": "为视频、直播、播客、游戏和商业内容生成纯音乐 BGM 与背景配乐。",
  "command": "node",
