...
**opencli 调用方式**：通过 `opencli web` 浏览器模式访问，无需登录即可使用基础功能。
...
# 查看暴涨词和新词（发现热点）
opencli web read "https://www.5118.com/ci"
# 页面中"最近暴涨词列表"和"最近新词列表"即为热点词
```

### 5118 多平台关键词挖掘（每个行业词必跑3个平台）

```bash
# Step 1：全网关键词（用户广义搜索习惯）
opencli web read "https://www.5118.com/ci?keyword=[行业词]"
# → 在页面平台选择器中切换为「全网」，截图或复制结果

# Step 2：抖音关键词（抖音用户搜索词）
opencli web read "https://www.5118.com/ci?keyword=[行业词]"
# → 切换为「抖音」，获取抖音平台高指数词

# Step 3：小红书关键词（小红书用户搜索词）
opencli web read "https://www.5118.com/ci?keyword=[行业词]"
# → 切换为「小红书」，获取小红书平台高指数词

# Step 4：淘宝/电商关键词（购买意图词）
opencli web read "https://www.5118.com/ci?keyword=[行业词]"
# → 切换为「淘宝」或「京东」，获取购买意图词
```

### 对应查询泛化（每个关键词必跑 3-5 个变体）

详细的查询泛化模板见 [QUERY_EXPANSION.md](QUERY_EXPANSION.md)
